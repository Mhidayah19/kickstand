  ...
  return (
    <SafeScreen scrollable showAppBar={false}>
      ...
      <View className="mb-6">
        ...
        <Text className="text-sm font-sans-medium text-charcoal/55 mt-1">{data.meta}</Text>
      </View>

      <HeroPedestal>
        <View className="p-8">
          <Text className="text-[10px] font-sans-bold tracking-atelier uppercase text-sand mb-3">
            12-month spend
          </Text>
        ...
        <Svg width="100%" height={96} viewBox="0 0 320 100" preserveAspectRatio="none">
          <Defs>
            <LinearGradient id="tyregrad" x1="0" y1="0" x2="0" y2="1">
              <Stop offset="0%" stopColor={colors.sand} stopOpacity="0.4" />
      ...
      <View className="bg-sand/10 rounded-3xl p-2 mb-8">
        {data.purchases.map((p, i) => (
          ...
          >
            <Text className="text-[14px] font-sans-bold text-charcoal" style={{ fontVariant: ['tabular-nums'] }}>
              {p.amount}
            </Text>
          </ListCard>
        ))}
...
function MetricCell({
  ...
  return (
    <View>
      <Text className="text-[9px] font-sans-bold tracking-atelier uppercase text-sand mb-1">
        {label}
      </Text>
