...
export const bikeSchema = z.object({
  ...
  year: z.coerce.number().int().min(1990, 'Year must be 1990 or later').max(currentYear + 1, 'Year is too far in the future'),
  plateNumber: z.string().min(3, 'Plate number must be at least 3 characters'),
  class: z.enum(['2B', '2A', '2']),
  currentMileage: z.coerce.number().min(0, 'Mileage must be 0 or greater'),
